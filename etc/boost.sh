...
#: ${BOOST_LIBS:="atomic chrono date_time exception filesystem program_options random regex signals system test thread"}
: ${BOOST_LIBS:="filesystem regex system"}
...
restoreBoost()
{
    cp $BOOST_SRC/tools/build/src/build/user-config.jam-bk $BOOST_SRC/tools/build/src/build/user-config.jam
}
...
updateBoost()
{
    ...
    cp $BOOST_SRC/tools/build/src/build/user-config.jam $BOOST_SRC/tools/build/src/build/user-config.jam-bk
 
    cat >> $BOOST_SRC/tools/build/src/build/user-config.jam <<EOF
...
buildFramework()
{
    ...
    $DEV_CMD lipo -create $BUILDDIR/*/libboost.a -o "$FRAMEWORK_INSTALL_NAME" || abort "Lipo $1 failed"
...
buildBoost
scrunchAllLibsTogetherInOneLibPerPlatform
buildFramework $IOSFRAMEWORKDIR $IOSBUILDDIR $ARM_DEV_CMD
buildFramework $OSXFRAMEWORKDIR $OSXBUILDDIR $OSX_DEV_CMD
